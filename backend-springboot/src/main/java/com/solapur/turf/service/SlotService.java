...
public class SlotService {
    ...
    public List<AvailabilitySlotDto> getAvailableSlots(UUID turfId, LocalDate date) {
        ...
        while (currentSlotTime.isBefore(closingTime)) {
            ...
            // 4. Skip slots that have FULLY passed today (e.g. 6AM slot at 11PM today)
            if (isToday && checkEnd.isBefore(now)) {
                currentSlotTime = slotEnd;
                continue;   // do not include expired slots in the response
            }
